export const comp1 = {
    "lioto": [
        {
            "title": "Em UBS  paciente com queixa de \"ouvido tapado\", zumbido e dificuldade para compreender. Na suspeita de cerume impactado é correto afirmar que:",
            "imagePath": null,
            "alternatives": [
                {
                    "option": "Como o diagnóstico é clínico, você agenda a lavagem auricular para algumas horas depois.",
                    "isTrue": false
                },
                {
                    "option": "Se a otoscopia visualiza perfuração timpânica, faz-se necessária a lavagem auricular com urgência.",
                    "isTrue": false
                },
                {
                    "option": "Na realização de remoção de cerume por irrigação, é fundamental realizar a otoscopia previamente e ao término do procedimento.",
                    "isTrue": true
                },
                {
                    "option": "Os critérios de indicação para a remoção de cerume são inspeção e palpação do ouvido.",
                    "isTrue": false
                },
            ]
        },
        {
            "title": "Cerume impactado é comum na atenção primária, podendo ocorrer em todas faixas etárias, principalmente em idosos. A remoção é indicada, exceto:",
            "imagePath": null,
            "alternatives": [
                {
                    "option": "Plenitude aural.",
                    "isTrue": false
                },
                {
                    "option": "Hipoacusia.",
                    "isTrue": false
                },
                {
                    "option": "Zumbido.",
                    "isTrue": false
                },
                {
                    "option": "Dor intensa.",
                    "isTrue": true
                }
            ]
        },
        {
            "title": "Sobre cuidados durante a remoção de cerume, é incorreto: ",
            "imagePath": null,
            "alternatives": [
                {
                    "option": "Sempre realizar com pressão moderada a forte a instilação do soro aquecido no ouvido acometido.",
                    "isTrue": true
                },
                {
                    "option": "A solução fisiológica deve ser aquecida até 37ºC para evitar tontura.",
                    "isTrue": false
                },
                {
                    "option": "Não insistir no procedimento na vigência e/ou persistência da dor.",
                    "isTrue": false
                },
                {
                    "option": "A irrigação pode ser realizada em pacientes que tenham utilizado gota otológica.",
                    "isTrue": false
                }
            ]
        },
        {
            "title": "Sobre cuidados durante a remoção de cerume, é incorreto: ",
            "imagePath": null,
            "alternatives": [
                {
                    "option": "Sempre realizar com forte pressão  a instilação do soro aquecido para garantir remoção adequada.",
                    "isTrue": true
                },
                {
                    "option": "A solução fisiológica deve ser aquecida até 37ºC para evitar tontura. ",
                    "isTrue": false
                },
                {
                    "option": "Não insistir no procedimento na vigência e/ou persistência da dor.",
                    "isTrue": false
                },
                {
                    "option": "A irrigação pode ser realizada em pacientes que tenham utilizado gota otológica.",
                    "isTrue": false
                }
            ]
        },
        {
            "title": "O cerúmen impactado é condição mais comum em:",
            "imagePath": null,
            "alternatives": [
                {
                    "option": "Crianças.",
                    "isTrue": false
                },
                {
                    "option": "Adultos.",
                    "isTrue": false
                },
                {
                    "option": "Idosos.",
                    "isTrue": true
                },
                {
                    "option": "Lactentes.",
                    "isTrue": false
                }
            ]
        },
        {
            "title": "Sobre remoção de cerúmen:",
            "imagePath": null,
            "alternatives": [
                {
                    "option": "É necessário realizar a otoscopia, antes e depois do procedimento.",
                    "isTrue": true
                },
                {
                    "option": "Otite aguda é indicação clara do procedimento.",
                    "isTrue": false
                },
                {
                    "option": "Em caso de perfuração timpânica, deve-se realizar o procedimento com urgência.",
                    "isTrue": false
                },
                {
                    "option": "O procedimento não pode ser realizado no contexto da Atenção Primária, devido aos riscos inerentes.",
                    "isTrue": false
                }
            ]
        },
        {
            "title": "Quando devo encaminhar para um especialista num caso de limpeza auricular?",
            "imagePath": null,
            "alternatives": [
                {
                    "option": "Perfuração timpânica.",
                    "isTrue": false
                },
                {
                    "option": "Tumoração",
                    "isTrue": false
                },
                {
                    "option": "Infecção associada.",
                    "isTrue": false
                },
                {
                    "option": "Todas as anteriores.",
                    "isTrue": true
                }
            ]
        },
        {
            "title": "Técnica de remoção de cerume mais segura em pacientes com membrana timpânica intacta?",
            "imagePath": null,
            "alternatives": [
                {
                    "option": "Lavagem do canal auditivo com jato de água fria.",
                    "isTrue": false
                },
                {
                    "option": "Remoção de cerume com pinça anatômica.",
                    "isTrue": false
                },
                {
                    "option": "Aspiração do cerume com instrumental apropriado.",
                    "isTrue": true
                },
                {
                    "option": "Remoção do cerume com cotonete.",
                    "isTrue": false
                }
            ]
        },
        {
            "title": "Complicações mais comuns da remoção de cerume com lavagem do canal auditivo?",
            "imagePath": null,
            "alternatives": [
                {
                    "option": "Perfuração da membrana e otite externa aguda. ",
                    "isTrue": true
                },
                {
                    "option": "Irritação da orelha média e vertigem.",
                    "isTrue": false
                },
                {
                    "option": "Otite externa e hipoacusia.",
                    "isTrue": false
                },
                {
                    "option": "Hemorragia do canal auditivo e tontura.",
                    "isTrue": false
                }
            ]
        },
        {
            "title": "Sobre lavagem auricular, assinale a incorreta.",
            "imagePath": null,
            "alternatives": [
                {
                    "option": "Presença de cerume é uma condição fisiológica  e confere proteção contra processos inflamatórios.",
                    "isTrue": false
                },
                {
                    "option": "A presença do cerume geralmente é assintomática, mas acúmulo indevido pode  gerar perda auditiva, dor e interferir na avaliação da membrana timpânica.",
                    "isTrue": false
                },
                {
                    "option": "A remoção de cerume é um procedimento comum na otorrinolaringologia e pode ser realizado na Atenção Primária à Saúde.",
                    "isTrue": false
                },
                {
                    "option": "Queixas de  plenitude aural, estalidos e diminuição da acuidade auditiva são sugestivos mas o diagnóstico de certeza é  feito através da  audiometria.",
                    "isTrue": true
                }
            ]
        }
    ],
    "devolutiva": "Nesta situação é necessário que saiba como e quando realizar remoção de cerumem  no atendimento na atenção primária.",
    "description": "Realizar remoção de cerume na atenção primária."
}